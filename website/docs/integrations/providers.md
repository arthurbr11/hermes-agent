...
| **Arcee AI** | `ARCEEAI_API_KEY` (direct) or `OPENROUTER_API_KEY` (via OpenRouter) in `~/.hermes/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
...
# Arcee AI (Trinity models — direct or via OpenRouter)
hermes chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY (direct) or OPENROUTER_API_KEY (via OpenRouter)
# Routing is auto-detected: direct key wins; sk-or- keys use OpenRouter.
```
